...
export function getOrderHistoryDetails(data) {
  return dispatch => {
    ...
    axios.post(urls.OrderHistoryDetail.url, data, header).then(response => {
      if (response.data.ack === '1') {
        dispatch(onSuccess(response.data, ORDER_HISTORY_DETAILS_DATA_SUCCESS))
      }
      else {
        dispatch(onFailure(response.data.msg, ORDER_HISTORY_DETAILS_DATA_ERROR))
      }
    })
      .catch(function (error) {
        dispatch(onFailure(strings.serverFailedMsg, ORDER_HISTORY_DETAILS_DATA_ERROR));
      });
  }
}
